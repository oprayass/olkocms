# OlkoCMS — Project Setup, Infra & Working-Method Findings
...
- `src/lib/prisma.ts` = PrismaClient singleton. `src/lib/auth.ts` = NextAuth config (single source of truth — see AUTH file). `src/middleware.ts` = protects `/dashboard/:path*`.
- Repo: https://github.com/oprayass/olkocms · Live: https://olkocms.vercel.app · Branch `main` (push = auto-deploy ~2-3 min).
...
- **Neon PostgreSQL** (free tier), Postgres 16. Host `ep-bold-moon-apfn4ql8-pooler.c-7.us-east-1.aws.neon.tech`, db `neondb`, user `neondb_owner`.
  ...
  - **Neon SQL Editor**: click **Run**, not "Explain Analyze" — Explain prefixes `EXPLAIN(...)` which makes DDL (`ALTER`/`CREATE`) throw `syntax error at or near "ALTER"`.
  - `prisma db execute --stdin` does NOT print SELECT output → inspect DB via a tiny debug API route + browser instead.
- **Vercel** (Hobby/free). Live `https://olkocms.vercel.app`.
  - Serverless timeout ~**10s** on Hobby even if a route sets `maxDuration=60` (ignored) → batch big loops (store-wise / status-wise / offset paginated).
  - Cron: max **2 jobs, daily only** (6-hourly not allowed). Current `vercel.json`: `expire-subscriptions` `0 0 * * *`, `orders/fetch` `0 1 * * *`.
  - Env-var changes do **not** auto-redeploy — push a commit or redeploy.
  - GitHub must be connected (Vercel → Settings → Git → Install GitHub App) for auto-deploy.
...
```
- `.env` and `.env.local` are git-ignored — created manually on each machine.
...
## Prisma
- **Use Prisma 5.22, NOT 7.x** — v7 schema format is incompatible; v5 stable. (`npm install prisma@5 @prisma/client@5`.)
- Build script: `"build": "prisma generate && next build"`. Also `"postinstall": "prisma generate"` (required for Vercel) — mind trailing commas or `package.json` parse fails on build.
...
- **Always**: `[System.IO.File]::WriteAllText("$PWD\path", $content, (New-Object System.Text.UTF8Encoding $false))`.
  - `$PWD\` prefix is **mandatory**: .NET uses the process working dir (often stuck at `C:\WINDOWS\System32`), not PowerShell's `cd`. Bare relative paths throw "Could not find a part of the path … System32". `Test-Path` may say True while .NET still fails.
- `Set-Content`/`Out-File -Encoding UTF8` add a **BOM** → Prisma schema (`P1012` on line 1) and JSON break. The `WriteAllText` + `UTF8Encoding $false` form is BOM-free.
...
- **PowerShell heredoc corrupts Nepali → `???`** — keep injected code/comments ENGLISH-ONLY; rewrite mangled `???`/`ðŸ"`/`à¤•à¥‹` strings in English. Emoji needs real unicode written via UTF-8, not escapes.
- For an alternative reliable writer, the `node fix.cjs` pattern (JS array of lines → `fs.writeFileSync`) also works.
...
### Path / shell quirks
- `Get-Content` treats `[id]` as a wildcard → use `Get-Content -LiteralPath`, or open bracketed paths with `code <file>`. `.NET WriteAllText` handles `[...]` fine.
- New folders need `New-Item -ItemType Directory -Force` before writing into them.
- **`fetch('/api/...')` runs in the BROWSER CONSOLE on the `olkocms.vercel.app` tab ONLY.** On claude.ai it hits `https://claude.ai/api/...` (404) or even a Stripe override (403). Use `fetch(window.location.origin + '/api/...')` if unsure. `npm`/`git`/`Select-String`/`$c=` are **PowerShell only** — pasting one into the other = SyntaxError.
- Do NOT paste TypeScript directly into PowerShell (parse errors) — write via `WriteAllText` or `code <file>`.

### Git
- Order matters: `git add .` → `git commit` → `git push`. PowerShell uses `;` not `&&`.
- Token URLs with `@github.com` break PowerShell splatting → wrap whole URL in quotes.
- `git push` rejected (fetch first) when GitHub changed elsewhere (e.g. schema edited in web UI) → `git pull --rebase` then push.
- `git pull` lock: "Unlink of file '.git/objects/pack/...' failed (y/n)" → answer `n` repeatedly, Fast-forward still completes; reboot clears the lock.
...
- Do **NOT** set `export const runtime = "edge"` on routes using Prisma — edge breaks Prisma (needs Node runtime).
- `useSession()` needs a `'use client'` `SessionProvider` wrapper in `dashboard/layout.tsx`; server components use `getServerSession`.
- ESLint/TS errors block builds → in `next.config.js`: `eslint:{ignoreDuringBuilds:true}`, `typescript:{ignoreBuildErrors:true}`.
...
- Local `npm run build` throws a harmless `PrismaClientInitializationError` with no local DB; production is fine because data routes are `force-dynamic`. Success line: `✓ Compiled successfully`.
- Next 14.2.x default `layout.tsx` Geist font import errors (`Unknown font 'Geist'`) → remove font import.
