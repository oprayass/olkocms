...
type StoreLite = { id: string; accessToken: string | null };

// Incremental DarazOrder write: new -> create, changed -> update, same -> skip.
// Runs inside a tenant context, so the scoped client injects subscriptionId
// into every where/create. If the same darazOrderId already exists under
// ANOTHER tenant, findUnique (tenant-scoped) sees nothing and create hits the
// global unique constraint -> P2002 -> skip, never overwrite across tenants.
...
// Fetch items+tracking for one order, trying its store first then the OTHER
// STORES OF THE SAME TENANT ONLY (callers pass tenant-local store lists).
...
type TenantNightlyReport = {
  ...
  step2_refreshStatus: { statusChanged: number };
  step3_resolveScans: { matched: number; stillUnmatched: number };
  step4_reconcile: { alertsCreated: number };
};

// All four nightly steps for ONE tenant. Must be called inside
// withExplicitTenant(subId, ...) so every prisma call is auto-scoped.
async function runNightlyForTenant(
  stores: StoreLite[],
  appKey: string,
  appSecret: string
): Promise<TenantNightlyReport> {
  ...
  return {
    step1_fetch: { ordersCreated, ordersUpdated, ordersSkipped },
    step1b_fillTracking: { trackingItemsSaved, ordersChecked: trackable.length },
    step2_refreshStatus: { statusChanged },
    step3_resolveScans: { matched, stillUnmatched },
    step4_reconcile: { alertsCreated },
  };
}
...
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    // Intentional cross-tenant read (prismaUnscoped): enumerate all active
    // stores once, group them by tenant, then run EVERYTHING ELSE inside
    // per-tenant withExplicitTenant scopes.
    ...
    for (const s of allStores) {
      const subId = (s as any).subscriptionId as string | null | undefined;
      if (!subId) { storesWithoutTenant++; continue; }
      const list = byTenant.get(subId);
      if (list) list.push(s);
      else byTenant.set(subId, [s]);
    }
    if (storesWithoutTenant > 0) report.storesWithoutTenant = storesWithoutTenant;

    const totals = {
      ordersCreated: 0, ordersUpdated: 0, ordersSkipped: 0,
      trackingItemsSaved: 0, statusChanged: 0,
      scansMatched: 0, scansStillUnmatched: 0, alertsCreated: 0,
    };
    const tenants: Record<string, unknown> = {};
    let tenantsFailed = 0;

    for (const [subId, stores] of byTenant) {
      try {
        const t = await withExplicitTenant(subId, () =>
          runNightlyForTenant(stores, appKey, appSecret)
        );
        tenants[subId] = t;
        totals.ordersCreated += t.step1_fetch.ordersCreated;
        totals.ordersUpdated += t.step1_fetch.ordersUpdated;
        totals.ordersSkipped += t.step1_fetch.ordersSkipped;
        totals.trackingItemsSaved += t.step1b_fillTracking.trackingItemsSaved;
        totals.statusChanged += t.step2_refreshStatus.statusChanged;
        totals.scansMatched += t.step3_resolveScans.matched;
        totals.scansStillUnmatched += t.step3_resolveScans.stillUnmatched;
        totals.alertsCreated += t.step4_reconcile.alertsCreated;
      } catch (err) {
        tenantsFailed++;
        tenants[subId] = { success: false, error: String(err).substring(0, 200) };
      }
    }

    report.tenantsProcessed = byTenant.size;
    report.tenantsFailed = tenantsFailed;
    report.totals = totals;
    report.tenants = tenants;
    report.success = tenantsFailed === 0;
    report.durationMs = Date.now() - startedAt;
    return NextResponse.json(report, { status: tenantsFailed === 0 ? 200 : 500 });
  } catch (err) {
